...
class MaskRNA:
    def __init__(self):
        self.input_size = 11          # grad(2) + whiskers(4) + probes(5)
        self.hidden_size = 14
        self.output_size = 3          # dx, dy, bite
    ...
    def decode_weights(self, bits):
        bytes_data = bits2bytes(bits, dtype="int8").astype(np.float32) / 10.0
        split_idx = self.w1_shape[0] * self.w1_shape[1]
        W1 = bytes_data[:split_idx].reshape(self.w1_shape)
        W2 = bytes_data[split_idx:].reshape(self.w2_shape)
        return W1, W2

    def forward(self, sensors, W1, W2):
        x_in = np.append(np.array(sensors, dtype=np.float32), 1.0)
        h = np.tanh(x_in @ W1)
        h_in = np.append(h, 1.0)
        return np.tanh(h_in @ W2)
...
def aplicar_modelo(imagem_arquivo, modelo_arquivo="criatura_melhor.npy"):
    ...
    grid_size = 22
    seeds = np.linspace(0.05, 0.95, grid_size)
    ...
    for state in agents:
        ...
        for _ in range(max_steps):
            ...
            radius_guess = np.clip(radius_guess + bite * 0.01, 0.03, 0.25)

            if bite > 0.45:
                r_eval = evaluate_bite(px, py, img)
                if r_eval > 0:
                    cx, cy = int(curr[0] * (W - 1)), int(curr[1] * (H - 1))
                    refined = magnetize_circle(cx, cy, int(r_eval), img)
                    if refined[3] > 0:
    ...
    for (x, y, r, _) in unique_balls:
        ...
        cv2.circle(res, (int(x), int(y)), 2, (0, 0, 255), -1)

    cv2.imwrite("resultado_usar_rna.png", res)
